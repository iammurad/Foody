    ...
    public class ProductsController : Controller
    {
            ...
            List<IFormFile>? productImages)
        {
            ...
            if (ModelState.IsValid)
            {
                    ...
                    if (firstImage != null && firstImage.Length > 0)
                    {
                        var existingMainPhoto = _productImageService.TGetAll()
                            .FirstOrDefault(x => x.ProductId == product.ProductId && x.IsMain);
                        existingMainPhoto.IsMain = false;
                        _productImageService.TUpdate(existingMainPhoto);
                        imagesToSave.Add((firstImage, true));
                    }
